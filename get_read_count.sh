...
then
echo -en "" > "raw_reads.count"
  for file in `find fastq -iname "*fq.gz"`
  do
    echo -en "$file\t" >> "raw_reads.count"
    echo `bioawk -c fastx 'BEGIN{counter=0;}{counter=counter+1;}END{print counter}' $file` >> "raw_reads.count"
  done
fi

if [ ! -f "filt_reads.count" ]
then
  echo -en "" > "filt_reads.count"
